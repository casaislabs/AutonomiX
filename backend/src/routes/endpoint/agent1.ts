...
export function registerAgent1Endpoint(app: Express): void {
  app.get('/api/agent1', async (_req: Request, res: Response) => {
    try {
      ...
      return res.status(200).json({ message: "hello, i'm Mercury", txHash: String(receipt?.hash || tx?.hash || ''), reputation: String(rep) })
    } catch (e: any) {
