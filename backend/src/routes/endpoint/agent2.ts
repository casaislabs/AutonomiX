...
export function registerAgent2Endpoint(app: Express): void {
  app.get('/api/agent2', async (_req: Request, res: Response) => {
    try {
      ...
      return res.status(200).json({ message: "hello, i'm Apollo", txHash: String(receipt?.hash || tx?.hash || ''), reputation: String(rep) })
    } catch (e: any) {
